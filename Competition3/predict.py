...
def removeStopWordsAndStem(data):
	...
	for i, row in data.iterrows():
		#strip HTML
		query = strip_tags(row['query'])
		title = strip_tags(row['product_title'])
		descript = strip_tags(row['product_description'])
		...
		descript = ' '.join([(word) for word in descript.split() if descript not in stop_words])
